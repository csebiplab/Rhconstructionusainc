...
const PagesSettings = () => {
  const {data,isLoading,error,mutate}=useSwr('/category',async(key)=>{
    try {
  ...
  return (
    <>
      ...
      {/* MODAL */}
      <PageForm onSuccess={()=>mutate(data)} category={editOrCreate} open={!!editOrCreate} handleClose={()=>setEditOrCreate(null)} />
    </>
  );
};
