  ...
  const handleSubmit = async () => {
    if (id) {
      ...
      try {
        ...
        if (res.status == 200) {
          toast(`Successfully updated ${endPoints} Meta data`);
          router.refresh();
      ...
      try {
        ...
        if (res.ok) {
          toast(`Successfully submitted ${endPoints} meta data`);
          router.refresh();
          router.push(`/dashboard/allpages/${endPoints}`);
        } else {
          throw new Error(`Failed to create ${endPoints} meta data`);
        }
      } catch (error) {
        console.log(error);
      }
            ...
            <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
              ...
              <div className="col-span-full">
                ...
                <p className="mt-3 text-sm leading-6 text-gray-600">
                  Write a few keywords about blogs.
                </p>
              </div>
            </div>
            <button
